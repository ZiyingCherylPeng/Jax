...
def vectorstore_exists(file_name):
    with DatabaseConnection(connection_pool) as cursor:
        cursor.execute("SELECT EXISTS(SELECT 1 FROM langchain_pg_collection WHERE name = %s)", (file_name,))
        exists = cursor.fetchone()[0]  
        return exists

def load_existing_vectorstore(collection_name, connection):
